...
class Menu extends CI_Controller
{
    ...
    public function submenuproses()
    {
        $post = $this->input->post(null, TRUE);
        $this->submenu_m->edit($post);
        if ($this->db->affected_rows() > 0) {
            echo "<script>
                alert('Data berhasil disimpan');
            </script>";
        ...
        echo "<script>
                window.location='" . site_url('menu/submenu') . "';
            </script>";
    }

    public function submenudel($id)
    {
        $this->submenu_m->del($id);
        if ($this->db->affected_rows() > 0) {
            echo "<script>
            window.location='" . site_url('menu/submenu') . "';
            </script>";
